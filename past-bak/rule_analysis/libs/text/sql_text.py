...
class SqlText(object):
    ...
    def __init__(self, mongo_client, start_date, stop_date, schema, hostname, record_id, db_client=None):
    ...
    def get_oracle_text(self):
        ...
        select = {"SQL_ID": 1, "SQL_TEXT_DETAIL": 1}
        records = self.mongo_client.find("sqltext", condition, select)
        sql_id_list = []
        sql_list = []
        sql_id_list = []
        for i in records:
            if i["SQL_ID"] not in sql_id_list and i["SQL_TEXT_DETAIL"]:
                sqlstat_list = []
