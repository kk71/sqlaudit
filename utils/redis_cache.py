...
def cache_it(limit=10000, expire=DEFAULT_EXPIRY, cache=None,
             use_json=False, namespace=None, type_to_exclude=None):
    """
    ...
    def decorator(function):
        ...
        def func(*args, **kwargs):
            ...
            if type_to_exclude:
                to_dump = [
                    [a for a in args if not isinstance(a, type_to_exclude)],
                    [k for k in kwargs.items() if not isinstance(k[1], type_to_exclude)]
                ]
            key = cache.get_hash(serializer.dumps(to_dump))
