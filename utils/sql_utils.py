...
def risk_sql_export_data(cmdb_id=None, schema=None,
    ...
    if ids:#决定导出所有或导出已选id
        ...
        # schema: list = list(set([a_schema
        #                    for a_schema in risk_sql.values_list("schema")]))
        # task_record_id: list = list(set([a_task_record_id
        #                          for a_task_record_id in risk_sql.values_list("task_record_id")]))
    ...
    with make_session() as session:
        ...
        if schema_task_record_id_rule:
            ...
            risk_sql_inners = get_risk_sql_list(
                cmdb_id=cmdb_id,
