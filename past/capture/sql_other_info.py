...
class CaptureOther(past.capture.base.Capture):

    def __init__(self, mongo_client, db_cursor, ipaddress, sid,
                 etl_date, startdate, stopdate, record_id, cmdb_id):
    ...
    def query_sql_plan(self, sql_id, hash_value, username, collection):
        v_sql_plan = SQL_PLAN_SET.replace("&sql_id", sql_id).replace("&h_value", hash_value)
    ...
    def query_no_bind(self, username):
        sql_no_bind = SQL_NO_BIND_SET.replace("&username", username)
    ...
    def query_sql_cursor(self, begin_snap_id, end_snap_id, username):
        sql_cursor = (SQL_CURSOR_SET.replace("&username", username)
                                    .replace("&beg_snap", begin_snap_id)
                                    .replace("&end_snap", end_snap_id))
    ...
    def query_no_bind_sql_info(self, sql):
        ...
        for data in records:
            ...
            """
            self.db_cursor.execute(temp_sql0)
    ...
    def query_snap_id(self):
        ...
        """
        self.db_cursor.execute(sql.format(startdate=self.startdate, stopdate=self.stopdate))
    ...
    def parse_result(self, username):
        ...
        snap_ids = self.query_snap_id()
        print("before - snap_ids: ", snap_ids)
        if not snap_ids:
            return
        if snap_ids[0] == 'None':
            return
        if snap_ids[0] == snap_ids[1]:
            end_snap_id = int(snap_ids[0])
            begin_snap_id = end_snap_id - 1
        else:
            begin_snap_id, end_snap_id = snap_ids
        # print("after - begin_snap_id: %d, end_snap_id: %d" % (int(begin_snap_id), int(end_snap_id)))

        begin_snap_id = str(begin_snap_id)
        end_snap_id = str(end_snap_id)
        ...
        for value in parameter:
            v_sql_id = (SQL_SET.replace("&beg_snap", begin_snap_id).replace("&end_snap", end_snap_id).replace("&parameter", value).replace("&username", username))
        ...
        for data in sql_id_set:
            tmp = data.split("#")
            self.query_sql_stat(begin_snap_id, end_snap_id, tmp[0], tmp[1], "sqlstat", username)
        ...
        sql_cursor = SQL_CURSOR_SET.replace("&username", username).replace("&beg_snap", begin_snap_id).replace("&end_snap", end_snap_id)
        ...
        for data in sql_cursor_data:
            ...
            past.rule_analysis.db.mongo_operat.MongoHelper.insert("stat_cursor", cursor_dict)
            self.query_sql_plan(data[1], str(data[2]), data[0], "stat_cursor_plan")
            self.query_sql_stat(begin_snap_id, end_snap_id, data[1], str(data[2]), "stat_cursor_stat", username)
