...
SELECT distinct owner  from (
      ...
      'SI_INFORMTN_SCHEMA', 'XDB', 'ODM','DBA','FLOWS_030000','DBMON','TIVOLI','SQLAUD','ADMIN','APEX_050100','OWBSYS','FLOWS_FILES','SCOTT',
      'APPQOSSYS','ORDDATA','ANONYMOUS','SQLAUDIT','APEX_030200','ZHANGXIN','ORACLE_OCM','WANGLEI',
      'YUSHUNQING','HONGWEIBIN','WANGLEI_ZR','LUHANG','ZHANGJUN','SHAOXIUQIN','TEST')   group by owner having count(*) >=5
      union
      select parsing_schema_name owner,count(*) count1 from dba_hist_sqlstat where
      ...
      'WMSYS', 'OLAPDBA', 'OLAPSVR', 'OLAP_USER',
      'OLAPSYS', 'EXFSYS', 'SYSMAN', 'MDDATA',
      'SI_INFORMTN_SCHEMA', 'XDB', 'ODM','DBA','FLOWS_030000','DBMON','TIVOLI',
      'SQLAUD','ADMIN','APEX_050100','OWBSYS','FLOWS_FILES','SCOTT','APPQOSSYS','ORDDATA','ANONYMOUS','SQLAUDIT','APEX_030200','ISQLAUDIT','ZHANGXIN','ORACLE_OCM',
       'WANGLEI','YUSHUNQING','HONGWEIBIN','WANGLEI_ZR','LUHANG','ZHANGJUN','SHAOXIUQIN','TEST')
       group by parsing_schema_name having count(*) >=5  ) aa
       order by 1  asc
"""
...
SELECT
    ...
    p.parent_id,
    p.operation,
    lpad(p.operation, ' ', 1 * p.depth) operation_display,
    p.options,
    p.object_node,
    p.object_owner,
    p.object_name,
...
WHERE username NOT IN (
  'XS$NULL','XDB','WMSYS','WKUSER','WKSYS','WKPROXY','SYSTEM','SYSMAN', 'IMONITOR', 'SPA', 'IDPAAS', 'IDATAAUDIT',
  'SYSKM','SYSDG','SYSBACKUP','SYS','SQLAUD','SPATIAL_WFS_ADMIN_USR','SPATIAL_CSW_ADMIN_USR', 'APPUSER4', 'APPUSER3',
  'SI_INFORMTN_SCHEMA','SCOTT','OWBSYS_AUDIT','OWBSYS','OUTLN','OSE$HTTP$ADMIN','ORDSYS', 'CMGPBI_REPORT',
  'ORDPLUGINS','ORDDATA','ORACLE_OCM','OLAP_USER','OLAPSYS','OLAPSVR','OLAPDBA','OJVMSYS', 'APPUSER2', 'EUS_ENGINE_USER',
  'ACSSEC', 'CLOUD_ENGINE_USER', 'CLOUD_SWLIB_USER', 'IDATACUBE',
  'OEM_MONITOR','ODM','MTSSYS','MGMT_VIEW','MDSYS','MDDATA','LBACSYS','JONES','ISQLAUDIT', 'SYSMAN_TYPES', 'SYSMAN_MDS',
  'SYSMAN_RO', 'SYSMAN_BIPLATFORM', 'SYSMAN_STB', 'SYSMAN_OPSS', 'TESTUSER', 'SPACEDBA',
  'GSMUSER','GSMCATUSER','GSMADMIN_INTERNAL','FLOWS_FILES','EXFSYS','DVSYS','DVF','DMSYS','DIP',
  'DBSNMP','CTXSYS','CLARK','BLAKE','AURORA$ORB$UNAUTHENTICATED''AURORA$JIS$UTILITY$','AUDSYS',
  'APPQOSSYS','APEX_PUBLIC_USER','APEX_INSTANCE_ADMIN_USER','APEX_050100',
  'APEX_040200','APEX_030200','ANONYMOUS','ADMIN','ADAMS','WMSYS','WK_TEST','SI_INFORMTN_SCHEMA',
  'ORDPLUGINS','OLAPSYS','LOGSTDBY_ADMINISTRATOR' ,'SYS', 'OUTLN', 'SYSTEM', 'CTXSYS', 'DBSNMP',
  'LOGSTDBY_ADMINISTRATOR', 'ORDSYS','ORDPLUGINS', 'OEM_MONITOR', 'WKSYS', 'WKPROXY','WK_TEST',
  'WKUSER', 'MDSYS', 'LBACSYS', 'DMSYS','WMSYS', 'OLAPDBA', 'OLAPSVR', 'OLAP_USER','OLAPSYS', 'EXFSYS',
  'SYSMAN', 'MDDATA','SI_INFORMTN_SCHEMA', 'XDB', 'ODM','DBA','FLOWS_030000','DBMON','TIVOLI','SQLAUD',
  'ADMIN','APEX_050100','OWBSYS','FLOWS_FILES','SCOTT','APPQOSSYS','ORDDATA','ANONYMOUS','SQLAUDIT','APEX_030200',
  'ZHANGXIN','ORACLE_OCM','WANGLEI','YUSHUNQING','HONGWEIBIN','WANGLEI_ZR','LUHANG','ZHANGJUN','SHAOXIUQIN',
  'TEST'
)
"""
